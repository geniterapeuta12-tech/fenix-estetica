...
--  Versão 12 — Execute no SQL Editor do Supabase → Run (idempotente: pode rodar de novo)
--  v12: armazém de arquivos (Storage) — fotos fora do limite do banco
...
-- v11: nunca bloqueia o cadastro ("Database error granting user") — falhas do vínculo são ignoradas
...
-- ============================================================================
-- v12 — ARMAZÉM DE ARQUIVOS (Storage): fotos fora da cota do banco
-- Bucket público de leitura; gravação só com sessão autenticada da clínica.
insert into storage.buckets (id,name,public,file_size_limit)
values ('fenix-arquivos','fenix-arquivos',true,15728640)
on conflict (id) do nothing;

drop policy if exists "fenix_foto_read" on storage.objects;
create policy "fenix_foto_read" on storage.objects
  for select using (bucket_id = 'fenix-arquivos');

drop policy if exists "fenix_foto_write" on storage.objects;
create policy "fenix_foto_write" on storage.objects
  for insert to authenticated with check (bucket_id = 'fenix-arquivos');

drop policy if exists "fenix_foto_update" on storage.objects;
create policy "fenix_foto_update" on storage.objects
  for update to authenticated using (bucket_id = 'fenix-arquivos')
  with check (bucket_id = 'fenix-arquivos');

drop policy if exists "fenix_foto_delete" on storage.objects;
create policy "fenix_foto_delete" on storage.objects
  for delete to authenticated using (bucket_id = 'fenix-arquivos');
